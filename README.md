...
本專題為一套具備「使用者身份驗證」與「教室借用管理」功能的 Web 應用系統，命名為 G16 教室管理系統，旨在改善傳統教室借用流程的效率與透明度，並提供易用且彈性的操作介面，供學生、教師與管理員使用。

系統採用模組化設計，主要拆分為以下兩大核心子系統：

- **🔐 身份驗證系統（Auth Service）**
 - 獨立的使用者註冊、登入與驗證模組，具備 JWT 驗證機制。
 - 支援權限分級管理（如：使用者 / 管理員），確保操作安全。
 - 可作為獨立服務被其他系統整合（具備擴充潛力與可重用性）。
