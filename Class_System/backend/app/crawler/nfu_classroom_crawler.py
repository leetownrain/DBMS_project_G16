...
def fetch_class_schedules():
    """
    利用 Selenium 與 BeautifulSoup 抓取教室課表資料，返回完整資料列表，
    並同時將完整與簡易版本分別寫入 JSON 檔案：
      - classroom_schedules_full.json：每筆包含「學年、學期、教室、星期、節次、課程、教授」
      - classroom_schedules_simple.json：每筆僅包含「學年、學期、教室、課程、教授」
    """
